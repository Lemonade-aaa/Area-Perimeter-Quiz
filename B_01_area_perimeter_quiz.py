...
Do math
you will be given random width and height, 
use them to calculate the perimeter and area.
All shapes are rectangles/squares for perimeter/area.
use code 'xxx' to exit the quiz at anytime.
you can choose to see the end results and summary at the end.
...
print("\n➕✖️🟰Welcome to MY perimeter area quiz🟰✖️➕\n")
print()
...
while quest_answered < num_questions:
    ...
    if area_quest == "true":
        ...
        #asks question,
        user_ans = (int_check(f"\nif the width is {width} and the height is {height}"
                        f" What is the area? ", exit_code="xxx"))

        # checks if user is correct
        if user_ans == area:
            quest_right += 1
            #determines if question is right
            right_wrong = "right"
...
if want_end_result == "yes":
    ...
    #calculates variables for summary
    percent_wrong = quest_wrong / quest_answered * 100
    percent_right = quest_right / quest_answered * 100
    average_percent = quest_wrong + quest_right / quest_answered * 100

    print(f"\nSummary: You got {quest_right} questions right"
          f" and {quest_wrong} questions wrong. "
          f"Percentages: Wrong {percent_wrong:.2f} Right: {percent_right:.2f} Average: {average_percent:.2f}\n")


    # tells user whether they were right/wrong for majority
    if percent_right < percent_wrong:
        print("You were wrong for the majority")

    elif percent_wrong < percent_right:
        print("You were right for the majority")

    elif percent_wrong == percent_right:
        print("Your as GREAT as you are BAD!")
